...
import { WHATSAPP_ASSISTANT } from "@/lib/constants/whatsapp"
    ...
    <div className="fixed bottom-6 right-6 z-[60] flex flex-col items-end gap-3">
      ...
      >
        <p className="text-sm font-medium text-foreground">
          {WHATSAPP_ASSISTANT.label}
        </p>
        <p className="mt-0.5 text-xs text-muted-foreground">
          {WHATSAPP_ASSISTANT.description}
        </p>
      </motion.div>

      <motion.a
        href={WHATSAPP_ASSISTANT.url}
        target="_blank"
        ...
        whileTap={{ scale: 0.95 }}
        aria-label={`${WHATSAPP_ASSISTANT.label} — abrir conversa no WhatsApp`}
        className={cn(
